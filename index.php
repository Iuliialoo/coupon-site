<?php 
    ...
    where `discount`.`id_category` = `category`.`id`;";
    $result = mysqli_query($connection, $query);
    $coupons = mysqli_fetch_all($result, MYSQLI_ASSOC);

    $count_coupons = "select count(`discount`.`id`) as count from `discount`, `category`
    where `discount`.`id_category` = `category`.`id`;";
    $result_count_coupons = mysqli_query($connection, $count_coupons);
    $count_coupon = mysqli_fetch_assoc($result_count_coupons);
?>
    <main>
        <h1>Всего скидок: <?php echo $count_coupon["count"]?></h1>
        <div class="coupons">
            <?php 
                foreach ($coupons as $coupon) {
                    $likes = $coupon["likes"] == null ? 0 : $coupon["likes"];
                    $img_src = $coupon["img"] == null ? "/src/img/discount.jpg" : "/server/img/".$coupon["img"];
                    ?>
                    <!-- <pre><?php print_r($coupon);?></pre> -->
                    <div class="coupon-item">
                        <img class="img-coupon" src="<?=$img_src?>" alt="" width="240" height="140"/>
                        <div class="info">
                            <p class="name-coupon"><?=$coupon["discount_name"]?></p>
                            <p class="value-coupon"><?=$coupon["value_discount"]?>%</p>
                            <p class="category-coupon">Категория: <?=$coupon["category_name"]?></p>
                            <p class="category-likes like">Лайк: <?=$likes?></p>
                            <a class="like-button" href="/?update_id=<?= $coupon['id'] ?>">Лайкнуть</a>
                            <a class="delete-button-main" href="/?delete_id=<?= $coupon['id'] ?>">Delete</a>
